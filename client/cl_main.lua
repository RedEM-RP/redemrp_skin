    ...
    while adding do
		...
		for i, v in ipairs(MaleComp) do
            ...
            elseif v.category == "mustache" then
				table.insert(malefhairs, v.Hash)
...
AddEventHandler('redemrp_skin:applySkin', function(skin)
    ...
    for i = 1 , 2 do
        ...
        SetPlayerModel(PlayerId(), model2, false)
        ...
        if sex == 1 then
            ...
            local fhairs = '0x' .. malefhairs[tonumber(_skin.fhair)]
            ...
            Citizen.Wait(300)
            Citizen.InvokeNative(0xD3A7B003ED343FD9, PlayerPedId(),tonumber(fhairs),true,true,true) -- Facial hair
            Citizen.Wait(300)
    ...
    if Config.usingClothing then
        print("load clothing")
        TriggerServerEvent("redemrp_clothing:loadClothes", 1, function(cb) end)
    else
...
RegisterNUICallback('changeBeard', function(data, cb)
    local fhair = data
    if sex == 1 then
        local fhairs = '0x' .. malefhairs[tonumber(fhair)]
        Citizen.InvokeNative(0xD3A7B003ED343FD9, PlayerPedId(),tonumber(fhairs),true,true,true)
    end
end)
